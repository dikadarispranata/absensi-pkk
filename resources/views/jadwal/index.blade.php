    ...
    <body>
        <div class="p-5">
            ...
            <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                ...
                <form method="GET" action="{{ route('jadwals.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-700">Cari Shift</label>
                        <input list="shiftList" name="nama_shift" 
                            ...
                            value="{{ request('nama_shift') }}">
                        <datalist id="shiftList">
                            @foreach ($jadwals as $jadwal)
                                <option value="{{ $jadwal->nama_shift }}">
                            @endforeach
                        </datalist>
                    </div>
                    ...
                    <table class="w-full border-collapse">
                        <thead class="bg-gray-50 border-b-2 border-gray-200">
                            <tr>
                                ...
                                <th class="p-4 text-sm font-semibold text-left text-gray-700">Nama Shift</th>
                                <th class="p-4 text-sm font-semibold text-left text-gray-700">Jam Masuk</th>
                                <th class="p-4 text-sm font-semibold text-left text-gray-700">Jam Keluar</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            @foreach ($jadwals as $jadwal)
                                <tr class="bg-white hover:bg-gray-50 transition duration-150">
                                    ...
                                    <td class="p-4 text-sm text-gray-700 whitespace-nowrap font-medium">{{ $jadwal->nama_shift }}</td>
                                    <td class="p-4 text-sm text-gray-700 whitespace-nowrap">{{ $jadwal->jam_masuk }}</td>
                                    <td class="p-4 text-sm text-gray-700 whitespace-nowrap">{{ $jadwal->jam_keluar }}</td>
    ...
    <div id="izinModal" class="fixed inset-0 z-[9999] hidden items-center justify-center bg-gray-900/60 backdrop-blur-sm">
        <div class="relative w-full max-w-md bg-white rounded-lg shadow-lg p-6 mx-4">
            <div class="mt-3">
                ...
                <form action="{{ route('jadwals.store') }}" method="POST" class="space-y-6 mt-4">
                    @csrf
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-700">Nama Shift</label>
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-700">Jam Keluar</label>
